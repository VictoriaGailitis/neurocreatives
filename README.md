...
4. [Умная предфильтрация постов (L0/L1/L2/L3)](#умная-предфильтрация-постов-l0l1l2l3)
...
- **Умная предфильтрация постов** ([`ai/prefilter.py`](ai/prefilter.py:1)) — 4-уровневый пайплайн L0→L1→L2→L3, отсекающий неподходящие посты от самого дешёвого этапа к самому дорогому (подробнее — [раздел ниже](#умная-предфильтрация-постов-l0l1l2l3)).
...
## Умная предфильтрация постов (L0/L1/L2/L3)

Чтобы не тратить деньги на анализ заведомо неподходящих постов, креативы проходят
четырёхуровневый пайплайн предфильтрации — от самого дешёвого этапа к самому дорогому.
Каждый следующий уровень запускается только для постов, прошедших предыдущий.

| Уровень | Где живёт | Когда выполняется | Что делает | Стоимость |
|---|---|---|---|---|
| **L0** — текст/метрики | [`ai/prefilter.py`](ai/prefilter.py:1) (`evaluate_text_metrics`) | в парсере, **ДО** скачивания картинки | отсев по длине текста, стоп-словам, просмотрам, ER и языку | бесплатно |
| **L1** — phash-дедуп | [`ai/image_similarity.py`](ai/image_similarity.py:1) (`find_duplicate_group_leader`, `collapse_into_leader`) | в анализе, до AI | сворачивает near-duplicate посты в **самый популярный** пост группы | дёшево |
| **L2** — AI-гейт | [`ai/image_classifier.py`](ai/image_classifier.py:1) (`ImageClassifier` + `evaluate`) | в анализе, перед полным разбором | дешёвая классификация (gpt-4o-mini): «реклама / не реклама», скриншоты текста, новостные фото, неэтичный контент | дёшево |
| **L3** — полный анализ | [`ai/image_analysis.py`](ai/image_analysis.py:1) (`analyze_image`) | финальный этап | подробное описание креатива через OpenAI Vision | дорого |

Единая точка входа для L1→L2→L3 — `ImageAnalyzer.analyze_all_unanalyzed()`
([`ai/image_analysis.py`](ai/image_analysis.py:246)); планировщик
([`scheduler/scheduler.py`](scheduler/scheduler.py:63)) вызывает именно её, поэтому
ни один пост не попадает в дорогой анализ в обход фильтра.

### Статусы постов и дубли
У каждого поста есть `prefilter_status`: `pending` → `passed` / `rejected` / `duplicate`,
а также `prefilter_stage` (на каком уровне отсеян) и `prefilter_reason` (причина).
Дубли **сворачиваются в самый популярный пост группы**: проигравшие получают
`duplicate_of = <id лидера>`, а лидер накапливает `duplicate_count`. Если позже
находится более популярный дубль — лидерство переходит к нему.

Посты, отсеянные на L0, сохраняются как лёгкие `rejected`-записи (без скачивания
картинки) — это нужно для аналитики воронки.

### Настройки (вкладка «Предфильтр»)
Все пороги заданы разумными дефолтами и редактируются в UI (⚙️ Настройки → вкладка
**«Предфильтр»**) либо через API. Ключи хранятся в таблице `Settings`:

| Поле UI | Ключ в `Settings` | Дефолт | Назначение |
|---|---|---|---|
| Включить предфильтр | `prefilter_enabled` | `true` | общий выключатель L0 |
| Мин. длина текста | `prefilter_min_text_len` | `30` | отсев слишком коротких постов |
| Стоп-слова | `prefilter_stopwords` | `розыгрыш, конкурс, подпишись, erid, вакансия, ищем сотрудника` | отсев по вхождению подстроки |
| Мин. просмотры | `prefilter_min_views` | `0` | порог по просмотрам (0 = выкл.) |
| Мин. ER | `prefilter_min_er` | `0` | порог по engagement rate (0 = выкл.) |
| Языки | `prefilter_languages` | `ru, en` | whitelist языков (пусто = любой) |
| Включить дедуп | `prefilter_dedupe_enabled` | `true` | общий выключатель L1 |
| Порог дедупа | `prefilter_dedupe_threshold` | `10` | макс. расстояние Хэмминга для near-duplicate |
| AI-гейт включён | `ai_filter_enabled` | `true` | общий выключатель L2 |
| Мин. solo appeal | `ai_min_solo_appeal` | `4.0` | минимальная привлекательность креатива |
| Отсев скриншотов текста | `ai_reject_text_screenshots` | — | резать скриншоты переписок/текста |
| Отсев новостных фото | `ai_reject_news_photos` | — | резать новостные фотографии |
| Отсев неэтичного | `ai_reject_unethical` | — | резать неэтичный контент |

### Метрики воронки
По каждому запуску парсера/анализа в `schedule_logs` пишутся счётчики
`rejected_l0`, `rejected_l1`, `rejected_l2`, `passed_full`. В UI они показываются
блоком «воронки» на вкладке «Предфильтр», а также доступны через API.

### API
| Метод | Эндпоинт | Назначение |
|---|---|---|
| `GET` | `/api/prefilter-settings` | текущие настройки L0/L1/L2 (с дефолтами) |
| `POST` | `/api/prefilter-settings` | сохранить любое подмножество полей |
| `GET` | `/api/prefilter-stats` | распределение постов по статусам + суммарная воронка |
| `POST` | `/api/prefilter-backfill` | идемпотентно разметить уже существующие посты |

### Backfill существующих постов
Кнопка **«Прогнать backfill»** на вкладке «Предфильтр» (или `POST /api/prefilter-backfill`)
прогоняет уже накопленные посты через L0+L1: проставляет статусы и сворачивает дубли.
Операция **идемпотентна** — обрабатываются только посты в статусе `pending`, поэтому её
безопасно запускать повторно. Возвращает `{"processed", "duplicates"}`.
...
> **После обновления на версию с предфильтром** миграции БД применятся
> автоматически на старте, но уже накопленные посты останутся в статусе
> `pending`. Чтобы разметить их через L0+L1 (статусы и сворачивание дублей),
> один раз запусти идемпотентный backfill:
> `curl -s -X POST http://localhost:8000/api/prefilter-backfill`
> либо нажми **«Прогнать backfill»** в UI → ⚙️ Настройки → вкладка «Предфильтр».
